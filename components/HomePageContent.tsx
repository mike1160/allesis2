...
const ctaPulseTransition = { duration: 2.3, repeat: Infinity, ease: easeOut };

const MotionLink = motion.create(Link);

const BRAND_BLUE = "#1a3bcc";
const GOLD = "#FFD700";

function AnimatedStarsRow() {
  return (
    <span className="inline-flex items-center gap-0.5" aria-hidden>
      {[0, 1, 2, 3, 4].map((i) => (
        <motion.span
          ...
          initial={{ opacity: 0.2, scale: 0.35 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true, margin: "-32px" }}
          transition={{ delay: i * 0.11, duration: 0.5, ease: easeOut }}
        >
          <motion.span
            className="inline-block text-[1.05em] leading-none"
            style={{ color: GOLD }}
            animate={{
              scale: [1, 1.12, 1],
              textShadow: [
                `0 0 8px rgba(255,215,0,0.5)`,
                `0 0 20px rgba(255,215,0,1), 0 0 32px rgba(255,215,0,0.55)`,
                `0 0 8px rgba(255,215,0,0.5)`,
              ],
            }}
            transition={{
...
const cardShellVariants = {
  rest: {
    y: 0,
    boxShadow: "0 12px 40px -12px rgba(10,15,30,0.12)",
  },
  hover: {
    y: -8,
    boxShadow: "0 28px 56px -14px rgba(26,59,204,0.32)",
    transition: { duration: 0.38, ease: easeOut },
  },
};

const gradientFadeVariants = {
  rest: { opacity: 0 },
  hover: {
    opacity: 1,
...
const gradientSpinVariants = {
  rest: { rotate: 0 },
  hover: {
    rotate: 360,
    transition: { repeat: Infinity, duration: 2.9, ease: "linear" as const },
  },
};
...
function ServiceCard({ d, index }: { d: Dienst; index: number }) {
  const [cardHovered, setCardHovered] = useState(false);

  return (
    <motion.div className="h-full" variants={cardEntranceVariants}>
      <motion.div
      ...
      >
        <motion.div
          ...
          variants={gradientFadeVariants}
        >
          <motion.div
            className="absolute left-1/2 top-1/2 h-[220%] w-[220%] -translate-x-1/2 -translate-y-1/2"
            style={{
              background: `conic-gradient(from 0deg, ${BRAND_BLUE}, #5b7cff, #7ca0ff, ${BRAND_BLUE})`,
            }}
        ...
        <motion.article
          variants={{ rest: {}, hover: {} }}
          className="group relative z-[1] flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border-2 border-transparent bg-white shadow-[0_12px_40px_-12px_rgba(10,15,30,0.12)] transition-colors hover:border-transparent"
        ...
        >
          <motion.div
            ...
            animate={{
              background: [
                "linear-gradient(125deg, rgba(26,59,204,0.9) 0%, transparent 42%, transparent 100%)",
                "linear-gradient(225deg, rgba(91,124,255,0.85) 0%, transparent 45%, transparent 100%)",
                "linear-gradient(320deg, rgba(34,211,238,0.75) 0%, transparent 40%, transparent 100%)",
                "linear-gradient(125deg, rgba(26,59,204,0.9) 0%, transparent 42%, transparent 100%)",
              ],
            }}
            transition={{ duration: 16, repeat: Infinity, ease: "linear", delay: index * 0.35 }}
          />
          <motion.div
            aria-hidden
            className="pointer-events-none absolute inset-y-0 -left-[40%] z-[2] w-[45%] skew-x-[-12deg] opacity-[0.4] group-hover:opacity-[0.85]"
            style={{
          ...
          />
          <div className="relative z-[3] flex flex-col p-8">
            <HomeServiceIconBlock titel={d.titel} index={index} cardHovered={cardHovered} />
            <h3 className="font-sora mt-2 text-[32px] font-bold text-neutral-dark">{d.titel}</h3>
            ...
            <MotionLink
              ...
              variants={{ rest: {}, linkHover: {} }}
            >
              Meer info{" "}
              <motion.span className="inline-block" variants={meerInfoArrowVariants} aria-hidden>
                →
              </motion.span>
            </MotionLink>
          </div>
...
export default function HomePageContent({ pakketten, diensten, reviews, renJiTags }: Props) {
  return (
    <motion.div
    ...
    >
          ...
          <div className="relative z-[1]">
            ...
            >
              {diensten.map((d, index) => (
                <ServiceCard key={d.titel} d={d} index={index} />
              ))}
            </motion.div>
          </div>
        </div>
      ...
      <RevealSection className="grain-dark relative overflow-hidden bg-neutral-dark px-6 py-[120px] md:px-10">
      ...
      {/* Sectie 5 — AVG Check */}
      <RevealSection className="bg-accent px-6 py-[120px] md:px-10">
      ...
      {/* Sectie 6 — Reviews */}
      <RevealSection className="relative overflow-hidden px-6 py-[120px] md:px-10">
        <div className="relative mx-auto max-w-6xl">
          <span
      ...
      {/* Sectie 7 — Hosting */}
      <RevealSection className="bg-neutral-light px-6 py-[120px] md:px-10">
      ...
      {/* Sectie 8 — CTA */}
      <RevealSection className="bg-primary px-6 py-[120px] md:px-10">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <MotionLink
            ...
            animate={{
              boxShadow: [
                "0 8px 24px -10px rgba(255,255,255,0.2)",
                "0 16px 40px -8px rgba(255,255,255,0.35)",
                "0 8px 24px -10px rgba(255,255,255,0.2)",
              ],
            }}
            transition={{ ...ctaPulseTransition }}
